...
class PanZoomWindow(object):
    ...
    def __init__(self, img, windowName = 'PanZoomWindow', onLeftClickFunction = None,onMiddleUpFunction=None,onMiddleDownFunction=None):
        self.WINDOW_NAME = windowName
        self.H_TRACKBAR_NAME = 'x'
        self.V_TRACKBAR_NAME = 'y'
        self.img = img
        self.onLeftClickFunction = onLeftClickFunction
        self.onMiddleUpFunction = onMiddleUpFunction
        self.onMiddleDownFunction = onMiddleDownFunction
    ...
    def onMouse(self,event, x,y,_ignore1,_ignore2):
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            ...
            if np.any(coordsInDisplayedImage < 0) or np.any(coordsInDisplayedImage > self.panAndZoomState.shape[:2]):
                ...
                coordsInFullImage = self.panAndZoomState.ul + coordsInDisplayedImage
                if self.onMiddleDownFunction is not None:
                    self.onMiddleDownFunction(coordsInFullImage[0],coordsInFullImage[1])
            ...
            if np.any(coordsInDisplayedImage < 0) or np.any(coordsInDisplayedImage > self.panAndZoomState.shape[:2]):
                ...
                coordsInFullImage = self.panAndZoomState.ul + coordsInDisplayedImage
                if self.onMiddleUpFunction is not None:
                    self.onMiddleUpFunction(coordsInFullImage[0],coordsInFullImage[1])
...
class PanAndZoomState(object):
 ...
 #       self.shape[:] = np.max(self.shape) 
        self.shape = np.maximum(PanAndZoomState.MIN_SHAPE,self.shape).astype(np.int) #prevent zooming in too far
